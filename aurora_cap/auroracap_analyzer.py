#!/usr/bin/env python3
"""
AuroraCap Analyzer for TikTok Production System
Integrates AuroraCap video captioning into the analysis pipeline
"""
import os
import sys
import json
...
class AuroraCapAnalyzer:
    """AuroraCap video description analyzer"""
    
    def __init__(self):
        self.name = "auroracap"
        self.aurora_path = "/home/user/tiktok_production/aurora_cap"
        self.script_path = os.path.join(self.aurora_path, "run_auroracap_simple.py")
        self._create_simple_inference_script()
        
    def _create_simple_inference_script(self):
        """Create a simplified inference script that works reliably"""
...
'''
    ...
    def analyze(self, video_path: str) -> Dict[str, Any]:
        """Analyze video using AuroraCap"""
        print(f"[AuroraCap] Analyzing video: {video_path}")
        ...
        try:
            ...
            cmd = [
                "bash", "-c",
                f"cd {self.aurora_path} && source activate_aurora_clean.sh && "
                f"python {self.script_path} '{video_path}' --output '{output_path}' --num_frames 8"
            ]
            
            # Run inference
            print("[AuroraCap] Running inference...")
            ...
            if result.returncode != 0:
                print(f"[AuroraCap] Error: {result.stderr}")
                return self._create_error_result(f"Inference failed: {result.stderr}")
            ...
            if os.path.exists(output_path):
                ...
                return self._create_error_result("Output file not created")
                
        except subprocess.TimeoutExpired:
            return self._create_error_result("Analysis timeout (300s)")
        except Exception as e:

        ...
        for frame_idx, (frame, timestamp) in enumerate(zip(frames, frame_times)):
            try:
                ...
                if len(frame_faces) == 0:
                    ...
                    face = frame_faces[0]
                    age_est = face.get('age_estimate', 'unbekannt')
                    gender = face.get('gender', 'unbekannt')
                    description = f"1 Person: {gender}, ca. {age_est} Jahre alt"
                else:
                    genders = [f.get('gender', 'unbekannt') for f in frame_faces]
                    ages = [f.get('age_estimate', 0) for f in frame_faces if isinstance(f.get('age_estimate'), int)]
                    avg_age = int(np.mean(ages)) if ages else 'unbekannt'
                    description = f"{len(frame_faces)} Personen: Durchschnittsalter {avg_age}, Geschlechter: {', '.join(set(genders))}"

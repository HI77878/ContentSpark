...
class ProductionEngine:
    def __init__(self):
        ...
        if self.use_staged_executor:
            logger.info("🚀 Using Staged GPU Executor for optimal memory management")
            self.executor = StagedGPUExecutor()
        else:
        ...
        if 'cross_analyzer_intelligence' in self.active_analyzers:
            ...
            logger.warning("✗ cross_analyzer_intelligence is MISSING from active_analyzers!")
            logger.warning(f"  - In GPU_ANALYZER_GROUPS? {'cross_analyzer_intelligence' in [a for group in GPU_ANALYZER_GROUPS.values() for a in group]}")
        ...
        try:
            if self.use_staged_executor:
                ...
                results = await loop.run_in_executor(
                    None,
                    self.executor.execute_all_stages,
                    video_path,
            ...
            # Run GPU cleanup after analysis
            try:
                cleanup_result = cleanup_gpu_memory()
                if cleanup_result.get('killed_pids'):
                    logger.warning(f"GPU Cleanup: Killed {len(cleanup_result['killed_pids'])} zombie processes")
                    logger.info(f"GPU Cleanup: Freed {cleanup_result.get('freed_memory_mb', 0):.0f}MB memory")
            except Exception as e:
